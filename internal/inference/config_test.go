...
func TestGatewayConfig(t *testing.T) {
	...
	body := `{"version":"gateway-config-v1","gateway_id":"local","base_url":"https://gateway.example/v1","credential_ref":{"kind":"file","path":"` + filepath.Join(dir, "credential") + `"},"protocols":["chat_completions"],"models":["model-a"]}`
	...
	for name, bad := range map[string]string{
		"unknown-protocol": strings.Replace(body, "chat_completions", "unapproved", 1),
		"relative-ca":      strings.Replace(body, `"gateway_id":`, `"ca_bundle":"relative.pem","gateway_id":`, 1),
		...
		"empty-protocols":  strings.Replace(body, `["chat_completions"]`, `[]`, 1),
	...
	if _, err := Start(context.Background(), Scope{}, nil); !errors.Is(err, ErrInvalidScope) {
