...
func LoadGatewayConfig(path string) (Gateway, error) {
	...
	if !list(c.Protocols, 16) || !list(c.Models, 256) {
		return Gateway{}, ErrGatewayConfig
	}
	...
	return Gateway{ID: c.ID, BaseURL: c.BaseURL, CredentialRef: c.CredentialRef, Protocols: c.Protocols, Models: c.Models, ProfileDigest: hex.EncodeToString(sum[:])}, nil
}
