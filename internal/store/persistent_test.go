...
func TestPersistentMigrationAndFixtureRefusal(t *testing.T) {
	...
	_, err = db.Exec("DROP TABLE execution_grant_heads; DROP TABLE execution_invalidations; DROP TABLE execution_grants; DROP TRIGGER events_no_update; DROP TRIGGER events_no_delete; ALTER TABLE metadata DROP COLUMN artifacts_dir; PRAGMA user_version=1")
	if err != nil {
		t.Fatal(err)
	}
	db.Close()
