...
func TestExecutionGrantSchemaThreeMigration(t *testing.T) {
	...
	if _, err = db.Exec("DROP TABLE execution_grant_heads; DROP TABLE execution_invalidations; DROP TABLE execution_grants; PRAGMA user_version=3"); err != nil {
		t.Fatal(err)
	}
	...
	defer r.Close()
	after := snapshot(t, r)
	if after.SchemaVersion != 4 || after.Generation != before.Generation || len(after.Events) != 0 {
		t.Fatal(after)
	}
