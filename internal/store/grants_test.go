...
func TestExecutionGrantSchemaTwoMigration(t *testing.T) {
	s, artifacts := persistent(t)
	before := snapshot(t, s)
	if err := s.Close(); err != nil {
		t.Fatal(err)
	...
	if _, err = db.Exec("DROP TABLE execution_grant_heads; DROP TABLE execution_invalidations; DROP TABLE execution_grants; PRAGMA user_version=2"); err != nil {
		t.Fatal(err)
	}
	...
	defer r.Close()
	after := snapshot(t, r)
	if after.SchemaVersion != 3 || after.Generation != before.Generation || len(after.Events) != 0 {
		t.Fatal(after)
	}
