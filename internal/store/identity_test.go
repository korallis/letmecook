...
func TestIdentityMigrationAndFixtureRefusal(t *testing.T) {
	...
	if _, err := db.Exec("DROP TABLE repository_profiles; DROP TABLE repositories; DROP TABLE execution_grant_heads; DROP TABLE execution_invalidations; DROP TABLE execution_grants; DROP TABLE credentials; DROP TABLE enrollments; DROP TABLE principals; PRAGMA user_version=2"); err != nil {
		t.Fatal(err)
	}
	db.Close()
	r, err := Open(ctx, s.dir, artifacts)
