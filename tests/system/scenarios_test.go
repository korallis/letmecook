...
func s01(t *testing.T, s *scenario) {
	...
	s.check(t, "temp-root refusal names rejected path and profile", strings.Contains(refused.Stderr, temp) && strings.Contains(refused.Stderr, "development verification") && strings.Contains(refused.Stderr, "0700") && strings.Contains(refused.Stderr, "system-temp"), refused)
	w.start("unqualified")
...
func s04(t *testing.T, s *scenario) {
	...
	s.check(t, "head verification and acceptance set", str(obj(task["head"])["receipt_id"]) != "" && boolean(obj(obj(task["verification"])["status"])["verified"]) && boolean(obj(task["review"])["accepted"]) && str(obj(obj(task["review"])["decision"])["action"]) == "accept", task)
...
func s05(t *testing.T, s *scenario) {
	...
	stopID := str(obj(stop["request"])["id"])
	s.require(t, "owner stop receipt identifies request", stopID != "", stop)
	...
	view := obj(r.get("/api/v1/stops/" + stopID + "?attempt_id=" + str(id["attempt_id"])))
	requested, requestErr := time.Parse(time.RFC3339Nano, str(obj(obj(view["Receipt"])["requested"])["wall"]))
	acknowledged, ackErr := time.Parse(time.RFC3339Nano, str(obj(obj(view["Evidence"])["measurement"])["acknowledged_at"]))
	delivery := acknowledged.Sub(requested)
	s.check(t, "runner cancellation acknowledged within one second", requestErr == nil && ackErr == nil && delivery >= 0 && delivery <= time.Second, object{"latency_ms": delivery.Milliseconds(), "stop": view, "clock": "co-located daemon and runner wall observation; not cross-host clock proof"})
	s.check(t, "stop confirms process termination and remote quiescence", str(view["ConfirmedProcess"]) == "terminated" && str(view["RemoteWork"]) == "quiescent", view)
