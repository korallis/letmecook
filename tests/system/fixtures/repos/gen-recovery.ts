...
const fixture = {
  ...
  timing_method: {
    ...
    timestamp_source:
      'The daemon events API carries no wall-clock timestamps; the harness records every timestamp from its own clock at the moment it observes the event and stores the event sequence beside it.',
    old_lease_barrier:
      'The measured recovery window deliberately includes the old-lease barrier: the 30 s maximum lease validity window (validity 20 s plus drift/termination margins) plus the daemon\'s retained 7 s issuance margin, 37 s as observed, whenever a pre-restart lease must lapse before its dispatch is releasable; no timing sample may subtract it.',
    unknown_is_blocking:
