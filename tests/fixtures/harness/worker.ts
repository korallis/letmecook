...
if (mode === 'oom') {
  ...
  await emitObservation({ ...result, artifact, requestsObserved: observed, ambientCanaries, processes: processObservation() });
  if (['partial', 'forbidden'].includes(mode)) {
    ...
    assert(['harness_failed', 'artifact_mismatch'].includes(result.outcome));
    assert(!result.events.some(event => event.type === 'tool_use'));
  } else assert.equal(result.outcome, expected[mode]);
