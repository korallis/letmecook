...
try{
 ...
 const staged = ['planner.ts','transport.ts','reader.ts','public-fixture.ts','router-consumer.ts','package.json'].map(f=>'experiments/planner-probe/'+f).concat(['json.ts','types.ts','protocol.ts','router-policy.ts'].map(f=>'experiments/inference-boundary/'+f),['tests/fixtures/planner/fixture.txt','tests/fixtures/planner/plan.schema.json']);
