...
for (const path of ['/router-source','/probe','/bridge','/planner-probe/router-gateway.ts','/consumer/experiments/planner-probe/router-gateway.ts','/consumer/experiments/inference-boundary/boundary.ts','/consumer/experiments/router-boundary-bridge','/work/router.sock','/work/journal/state.json','/var/run/docker.sock']) { await assert.rejects(access(path)); inaccessible.push(path); }
