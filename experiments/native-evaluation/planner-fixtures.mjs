...
export function plannerEvents(body,profile,scenario='read'){
 ...
 const call=(state.phase==='assessment'&&!repair&&!['direct','clarification'].includes(scenario))||scenario==='repair-tool'&&state.phase==='repair'||scenario==='disabled-call'&&state.phase!=='assessment';
 if(call){
 ...
 const proposal=plannerProposal(state.inputRevision,state.fileRead);if(scenario==='clarification')proposal.unresolved_questions=['Should the greeting retain its punctuation?'];
 let text=JSON.stringify(proposal);
