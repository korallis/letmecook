...
function setup() {
  ...
  observed.artifacts = [];
  const api = evidenceControls({ policy: () => p, gate: () => ({ snapshot: () => journal }), authority: { receipt: id => f.gateway.receipts.find(r => r.id === id), quiescent: () => true, evaluationScope: () => f.gateway.scope }, packetDigest: 'a'.repeat(64), observed, persist: (path, value) => writes.push({ path, value }) });
  const d = journal.decisions, value = { schema: 1, consumer: p.native.protocol, kind: 'repository_change', attemptId: d[0].attemptId, bindingDigest: digest(d[0].router.binding), policyDigest: digest(p), requestIds: d.map(d => d.requestId), decisionDigests: d.map(d => digest(d)), receiptDigests: d.map(d => d.evidence.receiptDigest), artifact: { path: 'greeting.txt', content: 'hello from probe\n', metadata: { source: 'constructed artifact fixture' } } };
...
test('private evidence and idempotent durable candidate ack preserve original receipt and decision links', () => {
  ...
  const ack = x.api.candidate(x.value); assert.deepEqual(ack, { acknowledged: true, digest: digest(x.value) }); assert.equal(x.writes.length, 1); assert.deepEqual(x.writes[0].value, { ...x.value, digest: ack.digest }); assert.equal(x.api.evidence(x.value.attemptId).artifacts[0].digest, ack.digest);
  assert.deepEqual(x.api.candidate(x.value), ack); assert.equal(x.writes.length, 1);
});
...
test('a failed durable artifact write returns no acknowledgement and publishes no artifact record', () => {
  const x = setup(), api = evidenceControls({ policy: () => x.p, gate: () => ({ snapshot: () => x.journal }), authority: { receipt: id => x.f.gateway.receipts.find(r => r.id === id), quiescent: () => true, evaluationScope: () => x.f.gateway.scope }, packetDigest: 'a'.repeat(64), observed: x.observed, persist: () => { throw Error('injected-fsync-failure'); } });
