...
const deniedPaths=['/state','/control/gateway.sock','/egress/provider.sock','/private/config.json','/router-source','/probe','/var/run/docker.sock','/root/.codex/auth.json','/root/.ssh','/root/.config/gh','/run/secrets','/Users'];
if(deniedPaths.some(existsSync))throw Error('worker_secret_or_control_mount');
if(Object.entries(process.env).some(([key,value])=>value&&/TOKEN|API_KEY|GITHUB|AWS_|GOOGLE|ANTHROPIC|OPENAI|PROXY/.test(key)))throw Error('worker_credential_environment');
const managementDenied=[];for(const path of ['/api/settings','/api/provider-connections','/api/combos','/control','/v1/chat/completions']){const status=await new Promise((resolve,reject)=>{const q=request({socketPath:'/router/inference.sock',path,method:'POST',headers:{host:'localhost',authorization:'Bearer '+grant.token,'content-type':'application/json'}},r=>{r.resume();r.once('end',()=>resolve(r.statusCode));});q.on('error',reject);q.setTimeout(1000,()=>q.destroy(Error('probe_timeout')));q.end('{}');});if(status!==404)throw Error('worker_management_reachable');managementDenied.push({path,status});}
...
console.log(JSON.stringify({event:'worker_finished',exit,captures,output:chunks.join(''),artifact:{path:'greeting.txt',content:readFileSync('/work/repo/greeting.txt','utf8')},containment:{tcpDenied,managementDenied,deniedPaths,workerOAuth:false,providerCredentials:false},config:{...config,provider:{openai:{...config.provider.openai,options:{...config.provider.openai.options,apiKey:'<boundary-grant>'}}}},environment:env}));
