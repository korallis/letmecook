...
const result={runId,startedAt:new Date().toISOString(),realProviderCalled:false,networkEgress:false,syntheticOriginalHttp:true,runtime:{server:version.Server,imageId,profile,cgroupVersion:info.CgroupVersion},cases:[],cleanup:[]};
writeFileSync(root+'/run.json',JSON.stringify(result,null,2)+'\n');
for(const scenario of (process.argv[3]?.split(',')??['roundtrip','slow-request','slow-scope','slow-token','slow-grant','admission-write','debit-write','receipt-write','altered-translation','override-rejectUnauthorized:false','override-dispatcher:null','override-agent:false','override-ca:null','override-key:""','override-cert:null'])){const name=scenario.replace(/[^a-z0-9-]/gi,'-'),file=scenario==='transport'?'../gaffer/experiments/native-evaluation/transport-proof.mjs':'native-local-integration.mjs';
 const container=runId+'-'+name; let created=false;
 try {
  docker(['create','--name',container,'--label',`${label}=${runId}`,'--pull','never','--network','none','--user','1000:1000','--cap-drop','ALL','--security-opt','no-new-privileges=true','--read-only','--init','--cgroupns','private','--ipc','private','--cpus','1','--memory','768m','--memory-swap','768m','--pids-limit','64','--ulimit','nofile=256:256','--ulimit','core=0:0','--shm-size','4m','--restart','no','--log-driver','local','--log-opt','max-size=2m','--log-opt','max-file=1','--log-opt','compress=false','--mount',`type=bind,source=${probe},target=/probe,readonly`,'--mount',`type=bind,source=${source},target=/router-source,readonly`,'--mount',`type=bind,source=${repository},target=/gaffer,readonly`,'--tmpfs','/egress:rw,nosuid,nodev,size=1m,uid=1000,gid=1000','--tmpfs','/tmp:rw,nosuid,nodev,size=180m,uid=1000,gid=1000','--env','HOME=/nonexistent','--env','NODE_OPTIONS=','--env',`DATA_DIR=/tmp/${name}`,'--env','GAFFER_SYNTHETIC_NATIVE=1','--env','GAFFER_TEST_FAULT='+scenario,image,'timeout','--signal=TERM','--kill-after=1s','40s','node','--experimental-loader','/gaffer/experiments/native-evaluation/fault-loader.mjs',`/probe/${file}`,scenario]); created=true;
  const before=JSON.parse(docker(['inspect',container]))[0]; const h=before.HostConfig;
  assert.equal(before.Image,imageId);assert.equal(before.Config.User,'1000:1000');assert.equal(before.Config.Labels[label],runId);assert.equal(h.NetworkMode,'none');assert.equal(h.Privileged,false);assert.equal(h.ReadonlyRootfs,true);assert.deepEqual(h.CapDrop,['ALL']);assert(!h.CapAdd?.length);assert.deepEqual(h.SecurityOpt,['no-new-privileges=true']);assert.equal(h.Init,true);assert.equal(h.PidMode,'');assert.equal(h.IpcMode,'private');assert.equal(h.CgroupnsMode,'private');assert.equal(h.Memory,768*1024*1024);assert.equal(h.MemorySwap,h.Memory);assert.equal(h.NanoCpus,1e9);assert.equal(h.PidsLimit,64);assert.equal(h.RestartPolicy.Name,'no');assert(!h.Devices?.length&&!h.DeviceRequests?.length&&!Object.keys(h.PortBindings??{}).length);assert.equal(before.Mounts.length,3);assert(before.Mounts.every(m=>m.Type==='bind'&&!m.RW));assert.deepEqual(h.Tmpfs,{'/tmp':'rw,nosuid,nodev,size=180m,uid=1000,gid=1000','/egress':'rw,nosuid,nodev,size=1m,uid=1000,gid=1000'});
  writeFileSync(root+'/'+name+'.inspect-before.json',JSON.stringify(before,null,2)+'\n');
  let stdout='',stderr='',executionError=null;
  try { stdout=docker(['start','--attach',container],45000); } catch(e){stdout=e.stdout??'';stderr=e.stderr??'';executionError=String(e.message);}
