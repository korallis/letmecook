...
export interface Request { schema: 1; fixture: typeof FIXTURE; baseSHA: string; settingsDigest: string; bindingDigest: string; token: string; wallMs: number; outputBytes: number }
export function validateRequest(r: Request) {
  keys(r, ['schema','fixture','baseSHA','settingsDigest','bindingDigest','token','wallMs','outputBytes'], ['schema','fixture','baseSHA','settingsDigest','bindingDigest','token','wallMs','outputBytes']);
  if (r.schema !== 1 || r.fixture !== FIXTURE || !/^[a-f0-9]{40}$/.test(r.baseSHA) || ![r.settingsDigest,r.bindingDigest,r.token].every(x => typeof x === 'string' && /^[a-f0-9]{64}$/.test(x)) || r.settingsDigest !== settingsDigest('ask') || !Number.isSafeInteger(r.wallMs) || r.wallMs < 1 || r.wallMs > 30000 || !Number.isSafeInteger(r.outputBytes) || r.outputBytes < 1 || r.outputBytes > 262144) throw Error('continuity_request');
}
...
export function start(input: Request) {
  ...
  const timer = setTimeout(() => { cancelled = true; terminate(); },r.wallMs);
