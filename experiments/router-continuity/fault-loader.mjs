...
export async function load(url,context,next) {
 ...
 if(!['pair','partial','outage','tool','intent-write','result-write'].includes(mode)||process.env.GAFFER_SYNTHETIC_NATIVE!=='1')throw Error('continuity_synthetic_only');
 source=replace(source,"res.writeHead(200,{'content-type':'text/event-stream'});const bytes=Buffer.from(frames(events(observed.sends.length===1)));for(let i=0;i<bytes.length;i+=17)res.write(bytes.subarray(i,i+17));res.end();",`res.writeHead(200,{'content-type':'text/event-stream'});let output=JSON.parse(JSON.stringify(events(${mode==='tool'})).replaceAll('Completed 🌍 café.','GAFFER_CONTINUITY_OK'));${mode==='partial'?'output=output.slice(0,5);':''}const bytes=Buffer.from(frames(output));for(let i=0;i<bytes.length;i+=17)res.write(bytes.subarray(i,i+17));res.end();`);
