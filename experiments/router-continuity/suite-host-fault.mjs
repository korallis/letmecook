...
if(!['storage','stop','unknown','consume-storage'].includes(mode))throw Error('suite_fault_opt_in_required');
registerHooks({load(url,context,next){const result=next(url,context);let source=String(result.source);
 if(url.endsWith('/native-evaluation/durable-records.mjs')&&['storage','consume-storage'].includes(mode))source=source.replace('export function persistImmutable(directory,kind,value){',`export function persistImmutable(directory,kind,value){
...
`);
 ...
 if(url.endsWith('/native-evaluation/deployment.ts')&&mode==='unknown')source=source.replace("'/probe/loader.mjs'","'/gaffer/experiments/router-continuity/suite-gateway-fault.mjs'");
