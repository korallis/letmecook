...
export async function consume(recordArgument:string,outputArgument:string,binary:string) {
 ...
 let stopRequested=false;const onStop=()=>{stopRequested=true;for(const worker of workers)void docker(['stop','--timeout','1',worker]).catch(()=>{});};for(const signal of ['SIGTERM','SIGINT'] as const)process.on(signal,onStop);
 const report:any={schema:1,fixture:FIXTURE,result:'pending',issueComplete:false,scopeCreated:false,workers:[],cleanup:false};
 const save=async()=>{const saved=persistImmutable(dirname(output),'continuity-consumer',report);await writeFile(output,JSON.stringify({snapshot:saved.file,digest:saved.digest,result:report.result})+'\n',{mode:0o600});};
 ...
 try{
  ...
  for(const attemptId of [FIRST,SECOND]){
   ...
   const state=await owned(worker);assert.equal(state.Image,record.runtime.image);const entry:any={binding,measured:inspectWorker(state,image,run,record.inferenceVolume,join(staging,'client'))};report.workers.push(entry);await save();
   ...
   }finally{clearTimeout(watchdog);if((await owned(worker)).State.Running)await docker(['stop','--timeout','1',worker]);entry.exited=await exited;entry.stderr=stderr;assert.equal((await owned(worker)).State.Pid,0);await save();}
 ...
 finally{for(const worker of workers)try{await owned(worker);await docker(['rm','--force',worker]);workers.delete(worker);}catch{}report.cleanup=!workers.size;await save();if(report.cleanup)await rm(staging,{recursive:true});for(const signal of ['SIGTERM','SIGINT'] as const)process.off(signal,onStop);}
