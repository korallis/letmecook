...
const all = ['edit', 'start-delay', 'entry-cancel', 'delay', 'ask', 'registry', 'cancel', 'crash', 'tree', 'receipt-write', 'decision-write', 'artifact-write', 'partial', 'forbidden', 'router-error', 'disabled-hook', 'ambient', 'auth', 'containment', 'transport', 'oom'];
...
try {
  ...
  for (const scenario of cases) {
    ...
    args.push('--env', 'DATA_DIR=/state/router-db', '--env', 'GAFFER_SYNTHETIC_NATIVE=1', '--env', 'GAFFER_TEST_FAULT=' + scenario, '--env', 'GAFFER_HARNESS_NATIVE_FAULT=' + (scenario === 'entry-cancel' ? 'cancel' : scenario), image, 'node', '--experimental-loader', '/gaffer/experiments/harness/native/fault-loader.mjs', '/gaffer/experiments/native-evaluation/bootstrap.mjs'); await docker(args);
    ...
    for (const [index, mode] of (scenario === 'registry' ? ['edit', 'ask'] : [scenario]).entries()) {
      ...
      const worker = run + '-' + scenario + '-worker-' + index; containers.add(worker); const argv = workerArgs(worker, run); argv.push('--interactive', '--mount', `type=bind,source=${clientStage},target=/fixture,readonly`, '--mount', `type=volume,source=${socketVolume},target=/router,readonly,volume-nocopy`, '--env', 'HOST_SENTINEL=/nonexistent-host-sentinel', image, 'node', '/fixture/experiments/harness/native/' + (['edit', 'start-delay', 'entry-cancel', 'delay', 'ask', 'receipt-write', 'decision-write', 'artifact-write', 'partial', 'forbidden', 'router-error'].includes(mode) ? 'worker.ts' : 'proof-worker.ts'), mode); await docker(argv);
      ...
      try {
        if (mode === 'oom') { entry.observation = await event(worker, 'native_worker_observation', 15000); assert(entry.observation.oom.after.oom_kill > entry.observation.oom.before.oom_kill); assert.equal(entry.observation.oom.exit.signal, 'SIGKILL'); entry.state = await stopWorker(worker); entry.outcome = 'cgroup_oom_observed'; }
          ...
          if (mode === 'entry-cancel') {
            const until = Date.now() + 15000; let active = false;
            while (Date.now() < until) { const e = await call(controlVolume, { command: 'evidence', attemptId: binding.attemptId }); if (e.receipts.some((r: any) => r.operations?.length)) { entry.beforeSignal = e; active = true; break; } await delay(50); }
            assert(active, 'native_original_not_started'); entry.signalAt = Date.now(); await docker(['kill', '--signal', 'TERM', worker]);
          }
          entry.observation = await event(worker, 'native_worker_observation'); await save();
          const artifact = mode === 'entry-cancel' ? null : JSON.parse(await docker(['exec', worker, 'node', '/fixture/experiments/harness/native/inspect-artifact.ts', baseSHA])); entry.observedArtifact = artifact;
          ...
          } else if (mode === 'entry-cancel') { assert.equal(entry.observation.result.outcome, 'cancelled_unknown'); assert(!entry.observation.result.events.some((e: any) => e.type === 'tool_use' && e.native.part.state.status === 'completed')); assert.equal(Number(await docker(['wait', worker], 1500)), 0); entry.stopMs = Date.now() - entry.signalAt; entry.outcome = 'cancelled_unknown'; entry.artifactObservation = 'unavailable_after_requested_stop'; } else if (mode === 'ask') { assert.equal(entry.observation.result.outcome, 'approval_blocked'); assert.equal(artifact.content, 'hello\n'); entry.outcome = 'approval_blocked'; }
