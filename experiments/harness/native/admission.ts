...
export function prepareRun(policy: RouterPolicy, binding: Binding, input: Omit<RunRequest, 'schema' | 'profile' | 'bindingDigest' | 'settingsDigest'>, now = Date.now()): RunRequest {
  ...
  if (p.schema !== 3 || p.native.protocol !== PROFILE || !p.native.harness || p.routerModel !== 'gpt-6-astra' || canonical(p.native.toolPaths) !== canonical(['greeting.txt'])) throw Error('unsupported_native_worker_policy');
