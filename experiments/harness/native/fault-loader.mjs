...
export async function load(url, context, next) {
  ...
  if (url.endsWith('/native-evaluation/gateway.mjs')) {
    ...
    source = replaceOne(source, "res.writeHead(200,{'content-type':'text/event-stream'});const bytes=Buffer.from(frames(n.protocol===PLANNER_PROTOCOL?plannerEvents(body,n):events(!body.input.some(x=>x.type==='function_call_output'))));for(let i=0;i<bytes.length;i+=17)res.write(bytes.subarray(i,i+17));res.end();", 'await sendFixture(res,events,frames,observed.sends.length,body);');
