...
export async function load(url, context, next) {
  ...
  if (url.endsWith('/native-evaluation/gateway.mjs')) {
    ...
    source = replaceOne(source, "res.writeHead(200,{'content-type':'text/event-stream'});const bytes=Buffer.from(frames(events(observed.sends.length===1)));for(let i=0;i<bytes.length;i+=17)res.write(bytes.subarray(i,i+17));res.end();", 'await sendFixture(res,events,frames,observed.sends.length);');
