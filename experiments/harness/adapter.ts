...
import { OPENCODE_PROFILE } from '../inference-boundary/profiles.ts';
...
export const describe = () => ({ schema: 1, adapter: 'opencode-m0-v1', harness: pins.version,
  nativeProtocol: pins.nativeProtocol, profile: OPENCODE_PROFILE, protocol: 'chat/completions',
...
export function probe(binary: string) {
  ...
  return { descriptor: describe(), binarySHA256: digest, phase: 'read-only-binary-integrity',
    capability: 'synthetic-runtime-proof-required', liveBlockers: [
      'No accepted live 9Router writer fence and request/provider quiescence authority',
...
export interface RunRequest {
  schema: 1; binding: Binding; profile: typeof OPENCODE_PROFILE;
...
export function config(token: string, approval: 'allow' | 'ask') {
  return { enabled_providers: ['scoped'], model: 'scoped/gaffer-coding', small_model: 'scoped/gaffer-coding', share: 'disabled', autoupdate: false, snapshot: false, formatter: false, lsp: false, mcp: {}, plugin: [], instructions: [], permission: { '*': 'deny', edit: approval },
    provider: { scoped: { npm: '@ai-sdk/openai-compatible', name: 'Scoped synthetic route', options: { baseURL: 'http://127.0.0.1:8787/v1', apiKey: token }, models: { 'gaffer-coding': { name: 'Synthetic bounded coding', tool_call: true, limit: { context: 8192, output: 128 }, modalities: { input: ['text'], output: ['text'] } } } } }, experimental: { openTelemetry: false } };
}
...
export function start(request: RunRequest): RunHandle {
  ...
  child = spawn('/fixture/opencode', ['run', '--format', 'json', '--model', 'scoped/gaffer-coding', '--title', 'Bounded synthetic fixture', request.brief], { cwd: '/work/repo', env: environment(), stdio: ['ignore', 'pipe', 'pipe'] });
