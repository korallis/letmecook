...
export class Boundary {
  ...
  private async handle(req: IncomingMessage, res: ServerResponse) {
    ...
    try {
      ...
      const decoder = new ChatStream(requestId, policy.routerModel, !!value.tools && value.tool_choice !== 'none', policy.profile === 'router-native-chat-translation-synthetic-v1' ? 'receipt-gated-eof' : policy.schema === 2 ? 'router-done' : 'done');
