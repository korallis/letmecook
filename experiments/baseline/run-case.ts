...
export async function runCase(directory: string, scenario: 'two-requests' | 'three-requests') {
  ...
  try {
    ...
    for(const suffix of ['state','router','control','repository']) { const volume=run+'-'+suffix; volumes.push(volume); await docker(['volume','create','--label',LABEL+'='+run,volume]);
    ...
    const [stateVolume,routerVolume,controlName,repositoryVolume]=volumes;controlVolume=controlName;report.stateVolume=stateVolume;
