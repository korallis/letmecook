...
func (m Metadata) Validate() error {
	if m.Version != Version || !(m.Mode == "fixture-only" && m.SchemaVersion == 1 || m.Mode == "store-only" && (m.SchemaVersion == 2 || m.SchemaVersion == 3 || m.SchemaVersion == 4 || m.SchemaVersion == 5 || m.SchemaVersion == 6)) || !p.ValidID(m.Generation) || !p.ValidID(m.DaemonBoot) || !slices.Equal(m.MissingCapabilities, MissingCapabilities()) {
