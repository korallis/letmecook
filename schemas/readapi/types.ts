...
export type Metadata = {
  ...
  generation: string; daemon_boot: string; schema_version: 1 | 2 | 3 | 4 | 5 | 6 | 7 | 8;
};
...
export function decode(bytes: Uint8Array, kind: 'status' | 'snapshot'): Status | Snapshot {
  ...
  require(v.version === VERSION && (v.mode === 'fixture-only' && v.schema_version === 1 || v.mode === 'store-only' && (v.schema_version === 2 || v.schema_version === 3 || v.schema_version === 4 || v.schema_version === 5 || v.schema_version === 6 || v.schema_version === 7 || v.schema_version === 8)));
